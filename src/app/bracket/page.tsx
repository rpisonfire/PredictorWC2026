...
import { STAGE_FIRST_MATCH, bracketStageFromLabel, treeRowFor, type BracketStage } from "@/lib/wc2026Bracket";
...
export default async function BracketPage() {
  ...
  // Buduj sloty: dla każdej fazy zmapuj mecze (sortowane po kickoff = FIFA numer) na rzędy drzewa
  ...
  const fillStage = (stage: BracketStage, list: typeof matches) => {
    ...
    list.forEach((m, idx) => {
      ...
      const row = treeRowFor(num);
      if (row === null) return;
      const arr = slots[stage as "r16" | "r8" | "qf" | "sf"];
      arr[row] = cast;
    });
  };
