  ...
  return (
    <section className="max-w-md mx-auto">
      ...
      <div className="flex justify-center mb-6">
        <PaniniCardLarge data={{
          nickname: user.nickname,
          avatar: user.avatar,
          rank: user.currentRank,
          totalPoints: stats.totalPoints,
          exactScoreHits: stats.exactScoreHits,
          scorerHits: stats.scorerHits,
          avgPointsPerMatch: stats.avgPointsPerMatch,
          badges,
          styleLabel: myStyle?.style.label,
          styleEmoji: myStyle?.style.emoji,
          championFlag: champ?.flag,
          championShortCode: champ?.shortCode,
        }} />
      </div>
      ...
      {/* Pozostałe statystyki - mini-tablica */}
      <div className="card p-5 mb-4">
        <div className="text-xs uppercase tracking-wider text-app-subtle mb-3">📊 Statystyki rozszerzone</div>
        <div className="grid grid-cols-3 gap-2">
          <Stat label="Najlepszy mecz" value={`${stats.bestMatchPoints}`} />
          <Stat label="Najdłuższa seria" value={stats.longestStreak} />
          <Stat label="Mistrzowskie ⚡" value={stats.successfulBoosts} />
        </div>
      </div>
